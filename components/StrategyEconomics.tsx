...
export function StrategyEconomics() {
  ...
  const metrics = useMemo(() => {
    ...
    const velyoRevenue = portfolioGmv * (fee / 100);
    const contribution = velyoRevenue * 0.4;
    ...
    const adrFor200kGmv = properties && occupancy
      ...
      : 0;
    const gmvFor200kVelyo = fee ? 200000 / (fee / 100) : 0;
    const occupancyFor200kVelyo = properties && adr && fee
      ? (gmvFor200kVelyo / (properties * adr * 30)) * 100
      : 0;
    return {
      occupiedNights,
      portfolioGmv,
  ...
  return <div className={styles.calculator}>
    ...
    <div className={styles.metricGrid}>
      ...
      <article><span>Ricavi Velyo / mese</span><strong>{euro.format(metrics.velyoRevenue)}</strong><small>commissione applicata al GMV</small></article>
      ...
      <article><span>Ricavi per immobile</span><strong>{euro.format(properties ? metrics.velyoRevenue / properties : 0)}</strong><small>prima dei costi fissi centrali</small></article>
    </div>
    ...
    <div className={styles.realityCheck}>
      ...
      <div className={metrics.occupancyFor200kVelyo > 100 ? styles.alert : undefined}>
        <span>Se 200.000 € significa ricavi Velyo</span>
        <strong>{metrics.occupancyFor200kVelyo.toFixed(0)}% di occupazione</strong>
        <p>Richiederebbe {euro.format(metrics.gmvFor200kVelyo)} di GMV mensile: non plausibile con {properties} immobili agli input attuali.</p>
      </div>
    </div>
